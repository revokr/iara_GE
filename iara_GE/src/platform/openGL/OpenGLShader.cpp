...
namespace iara {
	...
	OpenGLShader::OpenGLShader(const std::string& name, const std::string& filepath_vert, const std::string& filepath_geom, const std::string& filepath_frag) 
		: m_filepath_vert{ filepath_vert }, m_filepath_frag{ filepath_frag }, m_filepath_geom{ filepath_geom }, m_name{ name } {
		utils::createCacheDirIfNeeded();
		...
		if (!filepath_geom.empty()) {
			sources[GL_GEOMETRY_SHADER] = readFile(m_filepath_geom);
		}
	...
	void OpenGLShader::Compile_or_GetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shader_sources) {
		...
		for (auto&& [stage, source] : shader_sources) {
			...
			if (stage == GL_VERTEX_SHADER)
				...
				shader_file_path = m_filepath_geom;
			...
			if (in.is_open()) {
				...
				if (stage == GL_VERTEX_SHADER) input_filename = m_filepath_vert;
				else if (stage == GL_FRAGMENT_SHADER) input_filename = m_filepath_frag;
				else input_filename = m_filepath_geom;
				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, utils::getShaderCTypeFromGLSL(stage),
					input_filename.c_str(), options);
	...
	void OpenGLShader::Compile_or_GetOpenGLBinaries() {
		...
		for (auto&& [stage, spirv] : m_VulkanSPIRV) {
			std::filesystem::path shader_file_path;
			if (stage == GL_VERTEX_SHADER) shader_file_path = m_filepath_vert;
			else if (stage == GL_FRAGMENT_SHADER) shader_file_path = m_filepath_frag;
			else shader_file_path = m_filepath_geom;
	...
	void OpenGLShader::reload() {
		...
		{
			Timer timer;
			IARA_CORE_WARN("Reloading shader {0}", m_name);
			CompileToSPIRV(sources);
			CompileSPIRVtoOpenGL();
			CreateProgram();
			IARA_CORE_WARN("Shader reload took {0} ms", timer.elapsedMilliseconds());
		}
	}
	...
	void OpenGLShader::CompileToSPIRV(const std::unordered_map<GLenum, std::string>& shader_sources) {
		GLuint program = glCreateProgram();
	...
	void OpenGLShader::CompileSPIRVtoOpenGL() {
		shaderc::Compiler compiler;
		shaderc::CompileOptions options;
		options.SetTargetEnvironment(shaderc_target_env_opengl, shaderc_env_version_opengl_4_5);
	...
	void OpenGLShader::reflect(uint32_t stage, const std::vector<uint32_t>& shader_data) {
		...
		std::string file;
		if (stage == GL_VERTEX_SHADER) file = m_filepath_vert;
		else if (stage == GL_FRAGMENT_SHADER) file = m_filepath_frag;
		else file = m_filepath_geom;
		IARA_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", utils::glShaderToString(stage), file);
