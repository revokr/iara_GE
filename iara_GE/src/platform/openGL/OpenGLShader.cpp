...
namespace iara {
	...
	const std::string OpenGLShader::getName() {
		std::vector<uint32_t> vct;
		//spirv_cross::Compiler compiler(vct);
		return m_name;
	}
	...
	//	sources[GL_VERTEX_SHADER] = filepath_v;
	//	sources[GL_FRAGMENT_SHADER] = filepath_f;
	...
	//		std::filesystem::path cached_path = cache_dir / (shader_file_path.filename().string() + utils::getShaderStageExtensionVulkan(stage));
	...
	//			std::ofstream out(cached_path, std::ios::in | std::ios::binary);
	//			if (out.is_open()) {
	//				auto& data = shader_data[stage];
	...
	//		std::filesystem::path cached_path = cache_dir / (shader_file_path.filename().string() + utils::getShaderStageExtensionOpenGL(stage));
