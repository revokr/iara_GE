...
void iara::OrthographicCamera::setProj(float left, float right, float bot, float top) {
	m_proj = glm::ortho(left, right, bot, top, -1.0f, 1.0f);
	m_VP = m_proj * m_view;
}
...
iara::PerspectiveCamera::PerspectiveCamera(float width, float height, float degrees, float close, float farr) :
...
void iara::PerspectiveCamera::setProj(float width, float height, float degrees, float close, float farr)
{
	m_proj = glm::perspective(glm::radians(degrees), width / height, close, farr);
	m_VP = m_proj * m_view;
}

void iara::PerspectiveCamera::recalculateView() {
	IARA_PROFILE_FUNCTION();
