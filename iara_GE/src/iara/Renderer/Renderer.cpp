...
namespace iara {
	...
	void Renderer2D::drawAtmosphere(uint32_t vp_width, uint32_t vp_height, const glm::mat4& model_from_view, const glm::mat4& view_from_clip, const glm::mat4& view_from_model, const glm::mat4& clip_from_view, const glm::mat4& inverse_view, const glm::vec3& camera_pos, const glm::vec3& white_point, const glm::vec3& earth_center, const glm::vec3& sun_dir, const glm::vec3& sun_dir_y_up,
		...
		s_atmosphereData.atm_view_data_buffer.view_from_model = view_from_model;
		s_atmosphereData.atm_view_data_buffer.clip_from_view = clip_from_view;
		s_atmosphereData.atm_view_data_buffer.inverse_view = inverse_view;
		...
		RenderCommand::ActiveBindTexture2D(4, g_color);
		s_shaderLibrary->get("atmosphere")->setUniformInt("g_color", 4);
		...
		RenderCommand::ActiveBindTexture2D(7, g_normal);
		s_shaderLibrary->get("atmosphere")->setUniformInt("g_normal", 7);

		RenderCommand::ActiveBindTexture2D(8, ssao_map);
		s_shaderLibrary->get("atmosphere")->setUniformInt("ssao_map", 8);
	...
	void Renderer2D::depthPassAtmosphere(uint32_t vp_width, uint32_t vp_height, const glm::mat4& model_from_view, const glm::mat4& view_from_clip, const glm::mat4& view_from_model, const glm::mat4& clip_from_view, const glm::vec3& camera_pos, const glm::vec3& earth_center) {
		s_shaderLibrary->get("atmosphere_depth")->bind();

		s_atmosphereData.atm_data_buffer.camera = glm::vec4(camera_pos, 0.0);
		s_atmosphereData.atm_data_buffer.earth_center = glm::vec4(earth_center, 0.0);
		s_atmosphereData.atm_data_buffer.white_point = glm::vec4(0.0);
		s_atmosphereData.atm_data_buffer.sun_direction = glm::vec4(0.0);
		s_atmosphereData.atm_data_buffer.sun_size = glm::vec4(0.0);
		s_atmosphereData.atm_data_buffer.viewport_size = glm::vec4(vp_width, vp_height, 0.0, 0.0);
		s_atmosphereData.atm_data_buffer.exposure = 1.0f;
		s_atmosphereData.atm_data_uniform_buffer->setData(&s_atmosphereData.atm_data_buffer, sizeof(AtmosphereData::AtmData));
		...
		s_shadowMapData.shadow_quad_vao->bind();
		RenderCommand::drawArrays(s_shadowMapData.shadow_quad_vao, 0, 6);
		s_shaderLibrary->get("atmosphere_depth")->unbind();
	}
	...
	void MeshRenderer::InitMeshRenderer() {
		...
		for (int i = 0; i < 64; i++) {
			glm::vec3 sample(
				...
				rand01(rng) * 2.0f - 1.0f,
				rand01(rng)                 // z in [0,1] => hemisphere
			);
			...
			s_MeshData.ssao_kernel.push_back(glm::vec4(sample, 0.0f));
		}
