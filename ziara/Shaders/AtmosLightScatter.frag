...
#define SOFT_SUN 1
#define cloudy 0.25 //0.0 clear sky
...
float noise(in vec2 v2) 
{ 
    return textureLod(iChannel0, (v2 + 0.5) / 256.0, 0.).r; 
}

// by iq
float Noise(in vec3 v3)
{
    vec3 p = floor(v3);
    vec3 f = fract(v3);
	f = f * f * (3.0 - 2.0 * f);
...
float fnoise(vec3 p)
{
	...
	f = 0.5000 * Noise(p); p = p * 3.02; p.y -= ubuf.t*.1; //t*.05 speed cloud changes
	f += 0.2500 * Noise(p); p = p * 3.03; p.y += ubuf.t*.06;
...
float cloud(vec3 p) {
	float cld = fnoise(p*cloudnoise) + cloudy*0.1 ;
...
void densities(in vec3 pos, out float rayleigh, out float mie) {
	...
	if (mincloudheight < h && h < maxcloudheight) {
		//cld = cloud(pos+vec3(ubuf.t*1e3,0., ubuf.t*1e3))*cloudy;
        cld = cloud(pos + vec3(ubuf.t * 5e2, 0., ubuf.t * 6e2)) * cloudy; //direction and speed the cloud movers
		cld *= sin(3.1415*(h-mincloudheight)/mincloudheight) * cloudy;
	}
	#ifdef cloud2
        float cld2 = 0.;
        if (12e3 < h && h < 15.5e3) {
            cld2 = fnoise(pos*3e-4)*cloud(pos*32.0+vec3(27612.3, 0.,-ubuf.t*15e3));
...
{
    vec3 c = vec3(0.);
    float res = ubuf.iResolution.x*2.5;
...
float SawTooth(float tm) {
    return cos(tm+cos(tm))+sin(2.*tm)*.2+sin(4.*tm)*.02;
}

float DeltaSawTooth(float tm) {
    return 0.4*cos(2.*tm)+0.08*cos(4.*tm) - (1.-sin(tm))*sin(tm+cos(tm));
}  
...
vec2 GetDrops(vec2 uv, float seed, float m) {
    ...
    dropShape *= DeltaSawTooth(ubuf.t);
...
void main(){
	...
	float AR = ubuf.iResolution.x/ubuf.iResolution.y;
    float M = 1.0; //canvas.innerWidth/M //canvas.innerHeight/M --res
    
    vec2 uvMouse = (ubuf.iMouse.xy / ubuf.iResolution.xy);
    uvMouse.x *= AR;
    
   	vec2 uv0 = (fragCoord.xy / ubuf.iResolution.xy);
    uv0 *= M;
	...
	if (D.y < -ts) {
		...
		O = O + D * L;
        D.y = -D.y;
		D = normalize(D+vec3(0,.003*sin(ubuf.t+6.2831*noise(O.xz+vec2(0.,-ubuf.t*1e3))),0.));
		att = .6;
        star = stars(D);
    	...
    	vec3 D1 = vec3(1.);
    	D1 = normalize(D+vec3(1.,0.0009*sin(ubuf.t+6.2831*noise(O1.xz+vec2(0.,ubuf.t*0.8))),0.));
    	star = stars(D1);
